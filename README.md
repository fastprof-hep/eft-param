...
- Run the docker image ```source docker/run_image.sh```
- Within the image, set up the local environment ```source setup.sh```
- Set up EFT2Obs: ```cd EFT2Obs; source env.sh```
...
python scripts/get_scaling.py -c cards/zh-SMEFTsim3/config.json \
...
python scripts/makePlot.py --hist HiggsTemplateCrossSections_pT_V.json -c cards/zh-SMEFTsim3/config.json --x-title "p_{T}(Z) [GeV]" --title-left "qq #rightarrow Hl^{+}l^{-}" --title-right "SMEFTsim3" --ratio 0.9,1.95 --draw chw=0.1:4 chwb=0.1:2 chb=0.1:8 chbox=0.1:12 --show-unc --y-min 1E-9 --translate resources/translate_root.json
```
